...
### ParentInstanceId in trace and logs
...
> **Filtering:** For advanced filtering capabilities including GraphQL-style queries, logical operators, and aggregations, see [Instance Filtering Guide](/docs/how-to/instance-filtering).
...
### Instance envelope with metadata
...
> **Note:** Filtering works on instance attributes and is particularly useful when combined with pagination for large datasets. For more details on instance filtering, see the [Instance Filtering](https://github.com/burgan-tech/vnext-runtime/blob/main/README.md#instance-filtering) section.
...
The View function retrieves the appropriate view definition for the current workflow state. It supports platform-specific content, transition-specific views, and **remote (cross-domain) views**: views hosted in another domain can be referenced and used, enabling shared view reuse, versioning, and distribution.
...
**Content type:** `content` is typed by view `type`: when `type` is `Json`, `content` is an object or array; when `type` is `Html` (or similar), `content` is a string.
...
1. **Rule-based views**: If the state or transition defines a `views` array (rule-based view selection), the first matching rule determines the view. See [Rule-Based View Selection](/docs/how-to/view-selection) for details.
...
- [Custom Functions](/docs/components/functions/custom) - User-defined functions and Schema Function
- [Instance Filtering](/docs/how-to/instance-filtering) - GraphQL-style filtering guide
- [View Management](/docs/components/view) - View definitions and display strategies
- [State Management](/docs/components/workflow) - Understanding workflow states
- [Transition Management](/docs/components/mappings) - Executing transitions
- *Versioning and ETag (Phase 2)* - ETag pattern details