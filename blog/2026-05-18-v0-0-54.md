...
Scripts can now update **Stage** through the controlled **`ScriptContext.Mutations`** surface. This keeps platform-managed **Instance** fields protected while still allowing workflow-specific stage changes at intentional points inside **ScriptMapping** files:

```csharp
context.Mutations.SetStage("Account Created");
```

Stage can therefore be updated from an **Instance Start** or **transition request** payload, and it can also be advanced internally by scripts as the workflow reaches business milestones.
...
> **Referans:** [#641](https://github.com/burgan-tech/vnext/issues/641)
...
- [v0.0.54 Duyurusu](/blog/v0054-neler-degisti) — Feature-focused announcement in Turkish
